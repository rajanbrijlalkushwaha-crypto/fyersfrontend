...
export default function SplitChart() {
  ...
  useEffect(() => {
    ...
    if (historicalMode) {
      ...
      const _wsChartUrl = _apiBase ? _apiBase.replace(/^http/, 'ws') + '/ws/chart' : `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/ws/chart`;
      const ws = new WebSocket(_wsChartUrl);
      wsRef.current = ws;
