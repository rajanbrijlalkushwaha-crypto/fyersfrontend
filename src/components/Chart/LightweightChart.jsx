...
export default function LightweightChart({ symbol, expiry, date, historicalMode, cutoffTime, timeframe = 5 }) {
  ...
  useEffect(() => {
    ...
    const request = historicalMode
      ? fetchHistoricalCandles(symbol, expiry, date, timeframe)
      : fetchLiveCandles(symbol, timeframe);
    ...
    request
      .then(data => {
        ...
        if (!historicalMode) {
          ...
          const ws = new WebSocket(`${_wsBase}/ws/chart`);
          wsRef.current = ws;
          let wsConnected = false;

          ws.onopen = () => {
            wsConnected = true;
            ws.send(JSON.stringify({ type: 'subscribe', symbol: symbol.toUpperCase(), tf: timeframe }));
          };
  ...
  }, [symbol, expiry, date, historicalMode, cutoffTime, timeframe]);
