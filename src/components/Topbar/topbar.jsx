  ...
  return (
    <div className="topbar" id="mainTopbar" style={{ justifyContent: 'space-between', flexWrap: 'nowrap' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
        <span title={state.wsConnected ? 'Live WebSocket connected' : 'WebSocket disconnected — using HTTP fallback'}
          style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 700,
                   color: state.wsConnected ? '#22c55e' : '#ef4444' }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: state.wsConnected ? '#22c55e' : '#ef4444',
                         display: 'inline-block', boxShadow: state.wsConnected ? '0 0 5px #22c55e' : 'none' }} />
          {state.wsConnected ? 'LIVE' : 'OFF'}
        </span>
        <select
