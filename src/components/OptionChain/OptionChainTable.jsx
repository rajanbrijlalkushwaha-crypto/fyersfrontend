...
export default function OptionChainTable() {
  ...
  const [futuresMode, setFuturesMode]         = useState(false); // toggle spot vs futures reversal
  ...
  // S Level single click — selects cell and commits source
  ...
  // S Level double click — use futures reversal when in futures mode
  const handleSLevelDblClick = (e, spotReversal, futReversal) => {
    e.stopPropagation();
    const reversal = futuresMode && futReversal != null ? futReversal : spotReversal;
    if (reversal === null || reversal === undefined) return;
    if (singleClickTimerRef.current) {
      clearTimeout(singleClickTimerRef.current);
      singleClickTimerRef.current = null;
    }
    ...
    if (!src) return;
    const baseSpot = futuresMode && futuresLtp > 0 ? futuresLtp : currentSpot;
    dispatch({
      type: 'OPEN_LTP_POPUP',
      payload: { ...src, spot: baseSpot, targetReversal: reversal },
    });
  };
    ...
    <table id="optionTable" className={greeksActive ? 'show-greeks' : ''}>
      <thead>
        ...
        <tr className="header-sub">
          ...
          <th
            className={`call-sub data-col-cell slevel-header${futuresMode ? ' slevel-header-fut' : ''}`}
            style={{ cursor: 'pointer', userSelect: 'none' }}
            onClick={() => setFuturesMode(p => !p)}
            title={futuresMode ? 'Showing Futures Reversal — click to switch to Spot' : 'Click to show Futures Reversal'}
          >S Level{futuresMode ? ' (F)' : ''}</th>
          ...
          <th
            className={`put-sub data-col-cell slevel-header${futuresMode ? ' slevel-header-fut' : ''}`}
            style={{ cursor: 'pointer', userSelect: 'none' }}
            onClick={() => setFuturesMode(p => !p)}
            title={futuresMode ? 'Showing Futures Reversal — click to switch to Spot' : 'Click to show Futures Reversal'}
          >S Level{futuresMode ? ' (F)' : ''}</th>
      ...
      <tbody id="rows">
        {displayChain.map((r, idx) => {
          ...
          // S Level calculations — spot based
          let resistanceValue = null, supportValue = null;
          let ltpAtResistance = null, ltpAtSupport = null;
          // Futures-based reversal
          let futResistanceValue = null, futSupportValue = null;
          if (currentSpot > 0 && strikeGap > 0) {
            ...
            // Futures reversal (same formula, different spot price)
            const futSpot = futuresLtp > 0 ? futuresLtp : currentSpot;
            if (callUpper) futResistanceValue = calculateResistance(futSpot, r.put, callUpper.call);
            if (putLower)  futSupportValue    = calculateSupport(futSpot, r.call, putLower.put);
            ...
            <React.Fragment key={r.strike}>
              ...
              {showSpotRow && (() => {
                ...
                return (
                  <tr className="spot-row">
                    ...
                    <td colSpan={callCols - 1} className="spot-shift-side spot-shift-call-side">
                    ...
                    {/* Strike center — spans strike col + 1 S-Level col from each side = 3 cols */}
                    <td colSpan={3} style={{ padding: 0, border: 'none' }}>
                      <div className="spot-future-row">
                        {/* SPOT box */}
                        <div
                          className="spot-box spot-box-half"
                        ...
                        {/* FUTURE box */}
                        {futuresLtp > 0 && (
                          <div className="spot-box spot-box-half spot-box-future">
                            <span className="spot-label spot-label-future">FUTURE</span>
                            <span className="spot-value">{futuresLtp.toFixed(2)}</span>
                            {futuresChange !== 0 && (
                              <span className={`spot-diff ${futuresChange >= 0 ? 'spot-diff-up' : 'spot-diff-down'}`}>
                    ...
                    <td colSpan={putCols - 1} className="spot-shift-side spot-shift-put-side">
              ...
              <tr>
                ...
                {(() => {
                  ...
                  const displayVal = futuresMode ? futResistanceValue : resistanceValue;
                  const isBreakout = displayVal !== null && !isNaN(displayVal) && currentSpot > 0 && currentSpot > displayVal;
                  return (
                    <td
                      className={`data-col-cell slevel-cell slevel-clickable slevel-call ${isCallITM}${isSelected ? ' slevel-selected' : ''}${futuresMode ? ' slevel-fut-mode' : ''}${isBreakout ? ' slevel-broken' : ''}`}
                      onClick={e => handleSLevelClick(e, 'call', r.strike, r.call?.ltp, callDelta)}
                      onDoubleClick={e => handleSLevelDblClick(e, resistanceValue, futResistanceValue)}
                      title={futuresMode ? 'Futures reversal • Double-click to calculate' : 'Spot reversal • Double-click to calculate'}
                    >
                      <span className={`slevel-val${isBreakout ? ' slevel-val-struck' : ''}`}>{formatReversal(displayVal)}</span>
                      {isBreakout && <span className="slevel-brlabel slevel-breakout">Breakout</span>}
                ...
                {(() => {
                  ...
                  const displayVal = futuresMode ? futSupportValue : supportValue;
                  const isBreakdown = displayVal !== null && !isNaN(displayVal) && currentSpot > 0 && displayVal > currentSpot;
                  return (
                    <td
                      className={`data-col-cell slevel-cell slevel-clickable slevel-put ${isPutITM}${isSelected ? ' slevel-selected' : ''}${futuresMode ? ' slevel-fut-mode' : ''}${isBreakdown ? ' slevel-broken' : ''}`}
                      onClick={e => handleSLevelClick(e, 'put', r.strike, r.put?.ltp, putDelta)}
                      onDoubleClick={e => handleSLevelDblClick(e, supportValue, futSupportValue)}
                      title={futuresMode ? 'Futures reversal • Double-click to calculate' : 'Spot reversal • Double-click to calculate'}
                    >
                      <span className={`slevel-val${isBreakdown ? ' slevel-val-struck' : ''}`}>{formatReversal(displayVal)}</span>
                      {isBreakdown && <span className="slevel-brlabel slevel-breakdown">Breakdown</span>}
